# Databricks notebook source
# MAGIC %md-sandbox
# MAGIC
# MAGIC <div  style="text-align: center; line-height: 0; padding-top: 9px;">
# MAGIC   <img src="https://raw.githubusercontent.com/derar-alhussein/Databricks-Certified-Data-Engineer-Professional/main/Includes/images/deletes.png" width="60%">
# MAGIC </div>

# COMMAND ----------

# MAGIC %run ../Includes/Copy-Datasets

# COMMAND ----------

#These delete requests are received in the bronze table as CDC feed of type "delete" in the customers topic. While it is possible to process deletes at the same time as inserts and updates CDC feed, the fines around right to be forgotten requests may require a separate process.
#Here we indicate the time at which the delete was requested. And we add 30 days as a deadline to ensure compliance.
#In addition, we provide a field that indicates the current processing status of the request.


from pyspark.sql import functions as F

schema = "customer_id STRING, email STRING, first_name STRING, last_name STRING, gender STRING, street STRING, city STRING, country_code STRING, row_status STRING, row_time timestamp"

(spark.readStream
        .table("bronze")
        .filter("topic = 'customers'")
        .select(F.from_json(F.col("value").cast("string"), schema).alias("v"))
        .select("v.*", F.col('v.row_time').alias("request_timestamp"))
        .filter("row_status = 'delete'")
        .select("customer_id", "request_timestamp",
                F.date_add("request_timestamp", 30).alias("deadline"), 
                F.lit("requested").alias("status"))
    .writeStream
        .outputMode("append")
        .option("checkpointLocation", "dbfs:/mnt/demo_pro/checkpoints/delete_requests")
        .trigger(availableNow=True)
        .table("delete_requests")
)

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM delete_requests

# COMMAND ----------

# MAGIC %sql
# MAGIC --DELETE the delete records from customers_silver table
# MAGIC DELETE FROM customers_silver
# MAGIC WHERE customer_id IN (SELECT customer_id FROM delete_requests WHERE status = 'requested')

# COMMAND ----------

# MAGIC %md
# MAGIC Lets delete the deletes from customer_orders table

# COMMAND ----------

#To get the change data while reading the table, set the option readChangeFeed to true.
deleteDF = (spark.readStream
                 .format("delta")
                 .option("readChangeFeed", "true")
                 .option("startingVersion", 2)
                 .table("customers_silver"))

# COMMAND ----------

#below function deletes from customers_orders and updatesthe status of the delete request as "deleted" on delete_requests table
def process_deletes(microBatchDF, batchId):
    
    (microBatchDF
        .filter("_change_type = 'delete'")
        .createOrReplaceTempView("deletes"))

    microBatchDF._jdf.sparkSession().sql("""
        DELETE FROM customers_orders
        WHERE customer_id IN (SELECT customer_id FROM deletes)
    """)
    
    microBatchDF._jdf.sparkSession().sql("""
        MERGE INTO delete_requests r
        USING deletes d
        ON d.customer_id = r.customer_id
        WHEN MATCHED
          THEN UPDATE SET status = "deleted"
    """)

# COMMAND ----------

(deleteDF.writeStream
         .foreachBatch(process_deletes)
         .option("checkpointLocation", "dbfs:/mnt/demo_pro/checkpoints/deletes")
         .trigger(availableNow=True)
         .start())

# COMMAND ----------

# MAGIC %sql
# MAGIC --stataus is updated from requested to deleted
# MAGIC SELECT * FROM delete_requests

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE HISTORY customers_orders
# MAGIC --latest versions shows the DELETE operations done.
# MAGIC --note that version 3 would still have the deleted customers as delta lake supports time travel

# COMMAND ----------

# MAGIC %md
# MAGIC Actually the data is deleted from latest version and the data will still persist on the older versions. You need to use VACCUM to delete the requested customer from the delta table

# COMMAND ----------

# MAGIC %sql
# MAGIC --check if the deleted records present in v3 version of the table
# MAGIC SELECT * FROM customers_orders@v3
# MAGIC EXCEPT
# MAGIC SELECT * FROM customers_orders

# COMMAND ----------

#although the records are delted from customer table, deletes are still there in the CDF feed
df = (spark.read
           .option("readChangeFeed", "true")
           .option("startingVersion", 2)
           .table("customers_silver")
           .filter("_change_type = 'delete'"))
display(df)

# COMMAND ----------

# MAGIC %sql
# MAGIC describe history customers_silver
